.. _user:

*****************
User Guide
*****************

This chapter will guide the reader through the most common usage patterns of 
Ensemble Toolkit. We will start with Getting Started with the most basic 
Ensemble Toolkit example, and then add features to that example leading to a 
complete, more meaningful application.


.. toctree::
    
    user_guide/getstarted
    user_guide/adding_data
    user_guide/adding_instances
    user_guide/adding_stages
    user_guide/character_count
    user_guide/changing_remote
    user_guide/shared_data