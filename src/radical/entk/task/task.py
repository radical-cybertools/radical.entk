...
class Task(object):
    ...
    def from_dict(self, d):
        ...
        if d.get('pre_exec')            : self.pre_exec              = d['pre_exec']
        if d.get('executable')          : self.executable            = d['executable']
        if d.get('arguments')           : self.arguments             = d['arguments']
        if d.get('post_exec')           : self.post_exec             = d['post_exec']
        if d.get('cpu_reqs')            : self.cpu_reqs              = d['cpu_reqs']
        if d.get('gpu_reqs')            : self.gpu_reqs              = d['gpu_reqs']
        if d.get('lfs_per_process')     : self.lfs_per_process       = d['lfs_per_process']
        if d.get('upload_input_data')   : self.upload_input_data     = d['upload_input_data']
        if d.get('copy_input_data')     : self.copy_input_data       = d['copy_input_data']
        if d.get('link_input_data')     : self._link_input_data      = d['link_input_data']
        if d.get('move_input_data')     : self._move_input_data      = d['move_input_data']
        if d.get('copy_output_data')    : self._copy_output_data     = d['copy_output_data']
        if d.get('move_output_data')    : self._move_output_data     = d['move_output_data']
        if d.get('download_output_data'): self._download_output_data = d['download_output_data']
        if d.get('stdout')              : self._stdout               = d['stdout']
        if d.get('stderr')              : self._stderr               = d['stderr']
        if d.get('exit_code')           : self._exit_code            = d['exit_code']
        if d.get('path')                : self._path                 = d['path']
        if d.get('tag')                 : self._tag                  = d['tag']
        if d.get('parent_stage')        : self._p_stage              = d['parent_stage']
        if d.get('parent_pipeline')     : self._p_pipeline           = d['parent_pipeline']
