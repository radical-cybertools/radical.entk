...
class Task(object):
    ...
    def from_dict(self, d):
        ...
        if 'pre_exec'             in  d: self.pre_exec              = d['pre_exec']
        if 'executable'           in  d: self.executable            = d['executable']
        if 'arguments'            in  d: self.arguments             = d['arguments']
        if 'post_exec'            in  d: self.post_exec             = d['post_exec']
        if 'cpu_reqs'             in  d: self.cpu_reqs              = d['cpu_reqs']
        if 'gpu_reqs'             in  d: self.gpu_reqs              = d['gpu_reqs']
        if 'lfs_per_process'      in  d: self.lfs_per_process       = d['lfs_per_process']
        if 'upload_input_data'    in  d: self.upload_input_data     = d['upload_input_data']
        if 'copy_input_data'      in  d: self.copy_input_data       = d['copy_input_data']
        if 'link_input_data'      in  d: self._link_input_data      = d['link_input_data']
        if 'move_input_data'      in  d: self._move_input_data      = d['move_input_data']
        if 'copy_output_data'     in  d: self._copy_output_data     = d['copy_output_data']
        if 'move_output_data'     in  d: self._move_output_data     = d['move_output_data']
        if 'download_output_data' in  d: self._download_output_data = d['download_output_data']
        if 'stdout'               in  d: self._stdout               = d['stdout']
        if 'stderr'               in  d: self._stderr               = d['stderr']
        if 'exit_code'            in  d: self._exit_code            = d['exit_code']
        if 'path'                 in  d: self._path                 = d['path']
        if 'tag'                  in  d: self._tag                  = d['tag']
        if 'parent_stage'         in  d: self._p_stage              = d['parent_stage']
        if 'parent_pipeline'      in  d: self._p_pipeline           = d['parent_pipeline']
