	...
	def execute_pattern(self, pattern, resource):
		...
		try:
			if profiling == 1:
				...
				# CU execution profile logging -- RP profiler
				# ----------------------------------------------------------------------------------------------------------------------
				title = "uid, iter, step, Scheduling, StagingInput, AgentStagingInputPending, AgentStagingInput, \
					AllocatingPending, Allocating, ExecutingPending, Executing, AgentStagingOutputPending, \
					AgentStagingOutput, PendingOutputStaging, StagingOutput, Done"
				...
				# Required CU info
				u_frame.to_csv("execution_profile_{mysession}.csv".format(mysession=resource._session.uid), 
						sep=",")

		except KeyboardInterrupt:
			self._reporter.error('Profiling interupted')
			traceback.print_exc()
